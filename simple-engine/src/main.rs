...
use macroquad::prelude::{clear_background, draw_circle, draw_circle_lines, draw_line, draw_text, get_fps, is_mouse_button_down, mouse_position, next_frame, screen_height, screen_width, MouseButton, BLACK, WHITE, ORANGE, RED, Color};
use glam::Vec2;
...
async fn main() {
    ...
    let mut solver = Solver::new(
        ...
        Vec2::new(0.0, -9.8),  // Gravity
        constraint_radius,
    );

    let dt: u128 = 16;  // Fixed dt in milliseconds (approximately 0.55ms)
    println!("dt: {}", dt);
    ...
    loop {
        ...
        if is_mouse_button_down(MouseButton::Left) {
            if mouse_drop_accumulator >= mouse_drop_dt {
                let position = Vec2::new(mouse_position().0.into(), mouse_position().1.into()) - Vec2::new(screen_width as f32 / 2.0, screen_height as f32 / 2.0);
        ...
        if ball_drop_accumulator >= ball_drop_dt && !solver.is_container_full() {
            ...
            ballz.set_velocity(Vec2::new(0.0, 200.0), dt as f32 / 1000.0);
        ...
        while accumulator >= dt {
            solver.update(dt as f32 / 1000.0);  // Convert to seconds only when passing to physics
            accumulator -= dt;
            total_time += dt;

            if total_time >= 1 * 1000 && !determinism_done {
                ...
                if let Value::Object(ref mut object) = data {
                    if let Some(Value::Array(ref mut ball_array)) = object.get_mut("ball") {
                        ...
                        let x_str_inter = format!("{:.15}", solver.get_verlets()[0].get_interpolated_position((0.9-total_time as f32) / 0.815).x);
                        let y_str_inter = format!("{:.15}", solver.get_verlets()[0].get_interpolated_position((0.9-total_time as f32) / 0.815).y);
        ...
        for verlet in solver.get_verlets() {
            ...
            let origin = Vec2::new(screen_width as f32 / 2.0, screen_height as f32 / 2.0);
            let interpolated_pos = origin + verlet.get_interpolated_position(alpha as f32) * Vec2::new(1.0, -1.0);
